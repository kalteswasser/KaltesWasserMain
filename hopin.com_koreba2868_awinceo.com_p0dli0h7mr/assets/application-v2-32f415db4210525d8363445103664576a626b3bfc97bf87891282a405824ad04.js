...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleInput(e) {
    ...
    displayElement.innerText = `${valueLength}/${maxLength}${valueLength > maxLength && warning ? ` ${warning}` : ''}`;
    if (valueLength <= maxLength) {
      descriptionElement.innerText = `You have ${maxLength - valueLength} characters remaining`;
    }
    if (valueLength > maxLength) {
      descriptionElement.innerText = `You have ${valueLength - maxLength} characters too many.${
        warning ? ` ${warning}` : ''
      }`;
    }
  }
  ...
  if (characterCounts.length > 0) {
    characterCounts.forEach(input => {
      ...
      const displayElement = document.querySelector(`[data-count-display-for="${descriptionElementId}"]`);
      const maxLength = input.getAttribute('maxlength') || input.dataset.maxlength;
      const warning = input.dataset.warning;

      if (!descriptionElementId) {
        console.log(`Could not initiate character count input. No 'aria-describedby' specified`);
        return;
      }

      if (!descriptionElement) {
        console.log(`Could not initiate character count input. No description element matching 'aria-describedby'`);
        return;
      }
      ...
      if (!maxLength) {
        console.log(`Could not initiate character count input. No 'maxlength' attribute specified`);
        return;
      }

      const context = { descriptionElement, displayElement, maxLength, warning };
